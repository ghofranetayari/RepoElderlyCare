...
public class MonthlyFavoriteDoctorScheduler {
    ...
    /*
        ...
        public void applyOrRemoveProductPromotion() {
            ...
            boolean isDiscountMonth = currentMonth == Month.AUGUST || currentMonth == Month.MARCH;
    ...
    */
    ...
    public void checkAndUpdateBans() {
        ...
        Iterable<Elderly> elderlyList = elderlyRepository.findAll();

        for (Elderly elderly : elderlyList) {
            if (elderly.isBanned() && now.isAfter(elderly.getBannedUntil())) {
                elderly.setBanned(false);
                elderly.setBannedUntil(null);
